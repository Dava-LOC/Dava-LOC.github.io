jQuery(document).ready(function($) {
	...
	$('#epkb-admin__boxes-list__tools .epkb-kbnh__feature-links .epkb-primary-btn').on('click', function(){
		...
		$(this).closest('.epkb-admin__secondary-boxes-list').find('.epkb-kbnh__feature-container').css({'display' : 'none'});
		$(this).closest('.epkb-admin__secondary-boxes-list').find('.epkb-kbnh__feature-panel-container--' + id).css({'display' : 'block'});
	...
	$(document).on( 'epkb_hide_export_import_panels', function(){
		$('#epkb-admin__boxes-list__tools .epkb-admin__secondary-boxes-list>.epkb-kbnh__feature-container').css({'display' : 'flex'});
		$('#epkb-admin__boxes-list__tools .epkb-admin__secondary-boxes-list>.epkb-kbnh__feature-panel-container').css({'display' : 'none'});
	} );
	...
	$( '.epkb-admin__secondary-panel__item' ).on( 'click', function() {
		...
		let active_secondary_boxes_list_class = 'epkb-admin__secondary-boxes-list--active';
		...
		// Change class for active Boxes List
		$( '#epkb-admin__boxes-list__' + parent_list_key ).find( '.epkb-admin__secondary-boxes-list' ).removeClass( active_secondary_boxes_list_class );
		$( '#epkb-admin__secondary-boxes-list__' + list_key ).addClass( active_secondary_boxes_list_class );
		$( '#epkb-admin__boxes-list__' + parent_list_key ).find( '.epkb-admin__secondary-boxes-list' ).removeClass( 'epkb-admin__secondary-boxes-list--active' );
		$( '#epkb-admin__secondary-boxes-list__' + list_key ).addClass( 'epkb-admin__secondary-boxes-list--active' );
		$( '#epkb-admin__boxes-list__' + parent_list_key ).find( '.epkb-setting-box__list' ).removeClass( 'epkb-setting-box__list--active' );
		$( '#epkb-setting-box__list-' + list_key ).addClass( 'epkb-setting-box__list--active' );
	...
	(function(){
		...
		epkb.find( '#epkb_enable_advanced_search_debug' ).on( 'click', function() {
			...
			let postData = {
				...
				_wpnonce_epkb_enable_advanced_search_debug: $( '#_wpnonce_epkb_enable_advanced_search_debug' ).val()
			};
	...
	$( 'body' ).on( 'change', '#epkb-admin__secondary-boxes-list__settings__various [name=epkb_wpml_enable]', function(){
	...
	$( '.epkb-notification-box-basic__button-confirm' ).on( 'click', function () {
		if ( $( this ).attr( 'data-target' ).length > 0 ) {
			$( this ).closest( $( this ).attr( 'data-target' ) ).remove();
		}
	});

	// 'Explore Features' button on 'Need Help?' => 'Getting Started' page (possibly other similar links)
